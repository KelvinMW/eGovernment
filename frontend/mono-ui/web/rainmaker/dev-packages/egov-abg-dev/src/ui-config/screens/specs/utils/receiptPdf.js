...
export const generateMultipleBill = async (state, dispatch, type) => {
  ...
  if (index > -1) {
    ...
    const details =
      commonPayDetails &&
      commonPayDetails.filter((item) => item.code === "DEFAULT");
    billkey = get(details, "billKey", "");
  }
  allBills = allBills.filter((bill) => bill.status === "ACTIVE");
  allBills &&
    allBills.length > 0 &&
    (await downloadMultipleBill(allBills, billkey));
  /* 
...
export const downloadMultipleBills = async (
  bills = [],
  configKey,
  tenantId,
  locality,
    ...
    if (resp) {
      dispatch(
        toggleSnackbar(
          true,
          {
            labelName: "JOB_INITIATED_STATUS",
            labelKey: `${getLocaleLabels(
              "GRP_JOB_INITIATED_STATUS",
              "GRP_JOB_INITIATED_STATUS"
            )} ${resp.jobId}`,
          },
          "success"
...
export const generateSingleBill = async (billNo) => {
  ...
  let billData = {};
  const data = allBills.find((item) => get(item, "billNumber", "") === billNo);
  if (isEmpty(data)) {
              ...
              stack: [
                ...
                },
                {
                  stack: [
                    ...
                    {
                      text: `Tel. +252 ${transformedData.corporationContact}`,
                      style: "receipt-address",
                    },
                    {
          ...
          [
            {
              ol: [
                {
                  text: "5% rebate will be given if tax is paid for both half years at a time before 30th April of current financial year.",
                  margin: [8, 0, 0, 5],
                },
                {
                  text: "Property tax can be paid through: Cash or Cheque at eSeva, Mee Seva centres or Municipal Office or Online through Credit Card/Debitcard/Net Banking.",
                  margin: [8, 0, 0, 5],
                },
                {
                  text: "Due dates for payment of property tax without interest for current financial year: First half year 30th June, Second half year: 31st December.",
                  margin: [8, 0, 0, 5],
                },
                {
                  text: "A simple interest @ 2% per month will be charged in case of failure to pay property tax by due dates as above.",
                  margin: [8, 0, 0, 5],
                },
                {
                  text: "If the tax payers fail to pay the property tax within 15 days from the date of receipt of this Demand Bill, the same amount can be collected by issuing a distraint warrant u/s Section-269 of Municipal Corporations Act, 1955 (formerly GHMC Act, 1955).",
                  margin: [8, 0, 0, 5],
                },
                {
                  text: "Please bring this bill while paying at Office counter or eSeva or Mee Seva centers. ",
                  margin: [8, 0, 0, 10],
                },
              ],
              border: [true, false, true, true],
